odoo.define('ks_dashboard_ninja.ks_map_render_preview', function(require) {
    "use strict";
    ...
    var KsMapPreview = AbstractField.extend({
       ...
       get_map_data: async function(rec) {
            var self = this;
            ...
            if(this.$el.find(".map_text").length){
                this.$el.find(".map_text").remove();
            }
            ...
            if (chart_data.groupByIds){
                ...
                partners = await this._rpc({
                    model: "res.partner",
                    method: 'search_read',
                    args: [domain],
                    kwargs: {
                        fields: ["partner_latitude", "partner_longitude", "name"],
                    },
                });
            }
            ...
            var $chartContainer = $(QWeb.render('ks_dashboard_ninja.KsMapPreview', {
       ...
       _fetchRecordsPartner: async function(data) {
            let domain = [];
...
//            const records = await this.orm.searchRead("res.partner", domain, fields);
    ...
    registry.add('ks_dashboard_map_preview', KsMapPreview);

    return {
        KsMapPreview: KsMapPreview,
    };
});
